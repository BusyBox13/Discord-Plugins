 ...
 */
 ...
 module.exports = (() => {
     ...
     /* Configuration */
     const config = {info: {name: "My Meme Sounds", authors: [{name: "WarMachine#9792", github_username: "BusyBox13"}], version: "0.0.0.4", description: "Plays Memetastic sounds depending on what is being sent in chat.", github: "https://github.com/BusyBox13/Discord-Plugins/blob/main/MemeSound/MyMemeSound.plugin.js", github_raw: "https://raw.githubusercontent.com/BusyBox13/Discord-Plugins/main/MemeSound/MyMemeSound.plugin.js"}, defaultConfig: [{id: "setting", name: "Sound Settings", type: "category", collapsible: true, shown: true, settings: [{id: "LimitChan", name: "Limit to the current channel only.", note: "When enabled, sound effects will only play within the currently selected channel.", type: "switch", value: true}, {id: "delay", name: "Sound effect delay.", note: "The delay in miliseconds between each sound effect.", type: "slider", value: 200, min: 10, max: 1000, renderValue: v => Math.round(v) + "ms"}, {id: "volume", name: "Sound effect volume.", note: "How loud the sound effects will be.", type: "slider", value: 1, min: 0.01, max: 1, renderValue: v => Math.round(v*100) + "%"}]}], changelog: [{title: "MAJ", items: ["Tentative de mise à jour automatique du plugin", "contient 4 sons"]}]};
				...
				messageEvent = async ({ channelId, message, optimistic }) => {
					...
					if (!optimistic && lastMessageID != message.id) {
						...
						for (let sound of [...queue.entries()].sort((a, b) => a[0] - b[0])) {
							let audio = new Audio("https://github.com/BusyBox13/Discord-Plugins/raw/main/MemeSound/Sounds/"+sound[1].file);
